 ...
 *
 * 默认 --dry-run：只打印裁判消息。
 * --execute：MATCH_STARTED 时闭合夹手、起飞到 3 m、飞往 B 触发区；
...
constexpr float kTakeoffHeight = 1.0f;
...
constexpr float kBAltitude = 1.0f;
...
constexpr float kAAltitude = 1.0f;
...
constexpr float kAnswerAAltitude = 1.0f;
...
constexpr float kAnswerBAltitude = 1.0f;
...
constexpr float kAnswerCAltitude = 1.0f;

// 启动区：(x=0, z=0)。当前版本返回后保持 1 m 高度悬停。
constexpr float kStartLatitude = 39.07721710205078f;
constexpr float kStartLongitude = 119.71366882324219f;
constexpr float kStartAltitude = 1.0f;
...
bool takeoff(iking::drone::Client& client) {
    ...
    if (!isOk(client.takeOff(kTakeoffHeight, kRpcTimeoutMs), "takeOff(3m)")) {
        return false;
    }
...
const char* answerZoneName(PhysicalAnswerZone zone) {
    switch (zone) {
        case PhysicalAnswerZone::A: return "answer A";
        case PhysicalAnswerZone::B: return "answer B";
        case PhysicalAnswerZone::C: return "answer C";
    }
...
bool executeAnswerAndReturn(iking::drone::Client& client,
                            int round_index,
                            PhysicalAnswerZone target_zone) {
    print("[round] flying to answer zone B observation point");
    if (!flyToAnswerZone(client, PhysicalAnswerZone::B)) return false;
...
void eventLoop(iking::drone::Client& client, bool execute) {
    ...
    while (g_events.pop(event)) {
        ...
        if (event.message == "MATCH_FINISHED" ||
            ...
            answer_flow_started = true;
            std::this_thread::sleep_for(500ms);
            if (!captureScenePhoto(client, current_scene, round_index)) {
                print("[round] photo capture failed; continuing task");
            }
            // 识图逻辑接入前，暂时默认选择物理答题区 A。
            executeAnswerAndReturn(
                client, round_index, PhysicalAnswerZone::A);
            ...
            print("[match] question received; answer logic is not implemented");
        } else {
