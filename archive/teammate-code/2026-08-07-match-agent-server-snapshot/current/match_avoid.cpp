...
constexpr double kBArrivalToleranceMeters = 0.21;
...
constexpr double kAArrivalToleranceMeters = 0.21;


constexpr double kAnswerArrivalToleranceMeters = 0.21;
...
// ==================== 红柱避障 ====================
// 障碍线 x=(kZone2Longitude+kALongitude)/2 ≈ 2.7 m（东向偏移，距起点）。
// 红柱 500×500mm、高 2300mm，沿此线移动 ≤0.5 m/s。
constexpr double kObstacleLineX = 2.7;
constexpr double kCheckpointSpacingM = 1.0;         // 检查点距障碍线 1m
constexpr double kDroneHalfAxisM = 0.6;             // 飞机半轴 60cm
constexpr double kColumnHalfM = 0.25;               // 柱半宽 500mm/2
constexpr double kCorridorHalfM =
    kDroneHalfAxisM + kColumnHalfM + 0.15;          // ≈1.0m 穿越走廊半宽
constexpr double kCheckpointToleranceM = 0.12;      // 检查点到点容差（防过冲贴近柱体）
constexpr double kObstacleCamFOVDeg = 180.0;        // 前视鱼眼水平视场（近似，标定修正）
constexpr int    kObstacleDetectSamples = 4;        // 检查点采样帧数
constexpr int    kObstacleClearFrames = 5;          // 悬停中连续清晰帧数
constexpr double kObstacleHoverTimeoutS = 20.0;     // 悬停等清超时（秒）
constexpr int    kRedHueLo1 = 0,  kRedHueHi1 = 10;  // HSV 红色带1
constexpr int    kRedHueLo2 = 170, kRedHueHi2 = 179;// HSV 红色带2
constexpr int    kRedSatMin = 80, kRedValMin = 80;
constexpr double kColumnMinAspect = 1.5;            // 柱体高宽比下限
constexpr int    kColumnMinArea = 400;              // 红色连通域面积下限
...
// 将 lon/lat 转成相对参考点的局部坐标 (east, north)，单位米。
void toLocalMeters(double longitude, double latitude,
                   double ref_longitude, double ref_latitude,
                   double* east, double* north) {
    const double mean_latitude =
        degreesToRadians((latitude + ref_latitude) / 2.0);
    *east = degreesToRadians(longitude - ref_longitude) *
            kEarthRadiusMeters * std::cos(mean_latitude);
    *north = degreesToRadians(latitude - ref_latitude) *
             kEarthRadiusMeters;
}

// 局部坐标到经纬度（toLocalMeters 的反向）。
void fromLocalMeters(double east, double north,
                     double ref_longitude, double ref_latitude,
                     double* longitude, double* latitude) {
    const double mean_latitude = degreesToRadians(ref_latitude);
    *longitude = ref_longitude + east /
        (kEarthRadiusMeters * std::cos(mean_latitude)) / degreesToRadians(1.0);
    *latitude = ref_latitude + north / kEarthRadiusMeters / degreesToRadians(1.0);
}

// 起点到目标直线路径上，x（东向偏移）等于 x_target_m 处的插值点。
bool computePathPoint(double src_longitude, double src_latitude,
                      double dst_longitude, double dst_latitude,
                      double x_target_m, double* lon, double* lat) {
    double src_e, src_n, dst_e, dst_n;
    toLocalMeters(src_longitude, src_latitude,
                  kStartLongitude, kStartLatitude, &src_e, &src_n);
    toLocalMeters(dst_longitude, dst_latitude,
                  kStartLongitude, kStartLatitude, &dst_e, &dst_n);
    const double dx = dst_e - src_e;
    if (std::abs(dx) < 1e-9) return false;   // x 不变的竖直路径
    const double t = (x_target_m - src_e) / dx;
    if (t < -0.05 || t > 1.05) return false; // 不在路径段上
    const double e = src_e + t * dx;
    const double n = src_n + t * (dst_n - src_n);
    fromLocalMeters(e, n, kStartLongitude, kStartLatitude, lon, lat);
    return true;
}

// 方位角（0=正北，顺时针为正），用于机头对准穿越点。
double bearingDeg(double src_longitude, double src_latitude,
                  double dst_longitude, double dst_latitude) {
    double s_e, s_n, d_e, d_n;
    toLocalMeters(src_longitude, src_latitude,
                  kStartLongitude, kStartLatitude, &s_e, &s_n);
    toLocalMeters(dst_longitude, dst_latitude,
                  kStartLongitude, kStartLatitude, &d_e, &d_n);
    return std::atan2(d_e - s_e, d_n - s_n) * 180.0 / 3.14159265358979323846;
}
...
// 带障碍检查点的穿越航段飞行（定义见后）。
bool flyToAndHoverGuarded(iking::drone::Client& client,
                          const char* target_name,
                          double src_longitude, double src_latitude,
                          double dst_longitude, double dst_latitude,
                          double altitude, double final_yaw,
                          double arrival_tolerance);

bool flyToAAndHover(iking::drone::Client& client) {
    return flyToAndHoverGuarded(client,
                                "scene A",
                                kStartLongitude,
                                kStartLatitude,
                                kALongitude,
...
bool executeAnswerAndReturn(iking::drone::Client& client,
    ...
    PositionSnapshot obs_source;
    const bool have_source = latestPosition(obs_source);
    const bool obs_ok = have_source
        ? flyToAndHoverGuarded(client,
                               "zone 2 obs",
                               obs_source.longitude,
                               obs_source.latitude,
                               kZone23MidLongitude,
                               kZone23MidLatitude,
                               kZone23MidAltitude,
                               kMatchYaw,
                               kAnswerArrivalToleranceMeters)
        : flyToAnswerZone(client, PhysicalAnswerZone::Zone23Mid);
    if (!obs_ok) return false;
...
// ==================== 红柱避障：检查点识别 + 悬停等清 ====================
// 穿越障碍线（x=2.7m）的航段先飞至线两侧检查点（x=2.7±1m）悬停，
// 用前向相机识别红柱是否在穿越走廊内；在则悬停等清，超时返航。
// 机头以路径方位角对准穿越点，保证前向相机正对障碍方向。

// 抓一帧前向图像并转成 BGR。失败返回 false（按无障碍处理）。
bool grabFrontBgr(iking::drone::Client& client, cv::Mat& out) {
    iking::drone::FrameView frame;
    const iking::drone::StatusCode sc =
        client.acquireFrame(kSceneCameraChannel, frame, 200);
    if (sc != iking::drone::StatusCode::Ok) return false;
    if (frame.data && frame.width > 0 && frame.height > 0) {
        if (frame.type == kFrameBgr) {
            out = cv::Mat(frame.height, frame.width, CV_8UC3, frame.data).clone();
        } else if (frame.type == kFrameRgb) {
            ...
            cv::cvtColor(yuv, out, cv::COLOR_YUV2BGR_I420);
        }
    }
    client.releaseFrame(kSceneCameraChannel, frame);
    return !out.empty();
}

struct ObstacleSample {
    bool   detected = false;
    double center_x = -1.0;   // 红柱 bounding box 中心 x（像素）
    double width_px = 0.0;
};

// 从一帧 BGR 图像检测红色柱体（红色掩膜 + 竖条形状过滤）。
ObstacleSample detectRedColumn(const cv::Mat& bgr) {
    ObstacleSample out;
    ...
    cv::Mat red, m1, m2;
    cv::inRange(hsv, cv::Scalar(kRedHueLo1, kRedSatMin, kRedValMin),
                cv::Scalar(kRedHueHi1, 255, 255), m1);
    cv::inRange(hsv, cv::Scalar(kRedHueLo2, kRedSatMin, kRedValMin),
                cv::Scalar(kRedHueHi2, 255, 255), m2);
    ...
    for (const auto& c : contours) {
        ...
        if (area < kColumnMinArea) continue;
        const cv::Rect r = cv::boundingRect(c);
        if (r.height < kColumnMinAspect * r.width) continue;   // 竖条柱体
        if (r.height < h * 0.15) continue;                     // 高度需占画面足够比例
        if (area > best_area) { best_area = area; best = r; }
    }
    if (best_area <= 0.0) return out;

    out.detected = true;
    out.center_x = best.x + best.width / 2.0;
    out.width_px = best.width;
    return out;
}

// 柱体是否位于穿越走廊内。检查点距线 1m、柱体在线上，
// 走廊半宽 ≈1.0m 对应图像中心 ± 走廊角。
bool columnInCorridor(const ObstacleSample& s, const cv::Mat& bgr) {
    if (!s.detected || bgr.empty()) return false;
    const double deg_per_px = kObstacleCamFOVDeg / bgr.cols;
    const double corridor_deg =
        std::atan(kCorridorHalfM / kCheckpointSpacingM) * 180.0 / 3.14159265358979323846;
    const double half_px = corridor_deg / deg_per_px;
    return std::abs(s.center_x - bgr.cols / 2.0) <= half_px;
}

// 检查点处采样若干帧：红色柱体在走廊内 → 需要等待。
bool obstacleInPath(iking::drone::Client& client) {
    for (int i = 0; i < kObstacleDetectSamples; ++i) {
        cv::Mat bgr;
        if (!grabFrontBgr(client, bgr)) return false;  // 取帧失败按无障碍处理
        const ObstacleSample s = detectRedColumn(bgr);
        if (columnInCorridor(s, bgr)) return true;
        std::this_thread::sleep_for(50ms);
    }
    return false;
}

// 悬停等清：直到柱体离开走廊（连续 N 帧）或超时。
bool waitObstacleClear(iking::drone::Client& client) {
    const auto deadline = std::chrono::steady_clock::now() +
        std::chrono::milliseconds(static_cast<int>(kObstacleHoverTimeoutS * 1000));
    int clear_frames = 0;
    while (std::chrono::steady_clock::now() < deadline) {
        if (g_stop.load() || g_urgent_return.load()) return false;
        cv::Mat bgr;
        if (grabFrontBgr(client, bgr)) {
            const ObstacleSample s = detectRedColumn(bgr);
            if (!columnInCorridor(s, bgr)) {
                if (++clear_frames >= kObstacleClearFrames) return true;
            } else {
                clear_frames = 0;
            }
        }
...
// 检查点守护：柱体在走廊内则悬停等清；超时返航。
bool guardCheckpoint(iking::drone::Client& client) {
    if (!obstacleInPath(client)) return true;
    print("[avoid] red column in corridor; hovering and waiting for clear");
    if (waitObstacleClear(client)) {
        print("[avoid] corridor clear; resuming");
        return true;
    }
    print("[avoid] obstacle did not clear in time; returning home");
    returnHome(client, "obstacle_timeout");
    return false;
}

// 穿越障碍线航段：经 x=1.7 / x=3.7 两个检查点悬停识别，放行后继续。
// 机头以路径方位角对准穿越点，保证前向相机正对障碍方向。
bool flyToAndHoverGuarded(iking::drone::Client& client,
                          const char* target_name,
                          double src_longitude, double src_latitude,
                          ...
                          double altitude, double final_yaw,
                          double arrival_tolerance) {
    double src_e = 0, src_n = 0, dst_e = 0, dst_n = 0;
    toLocalMeters(src_longitude, src_latitude,
                  kStartLongitude, kStartLatitude, &src_e, &src_n);
    toLocalMeters(dst_longitude, dst_latitude,
                  kStartLongitude, kStartLatitude, &dst_e, &dst_n);
    ...
    // 路径不跨越障碍线 → 原样直飞
    if ((src_e - kObstacleLineX) * (dst_e - kObstacleLineX) >= 0.0) {
        return flyToAndHover(client, target_name, dst_longitude, dst_latitude,
                             altitude, final_yaw, arrival_tolerance);
    }

    double cp_west_lon, cp_west_lat, cp_east_lon, cp_east_lat;
    const bool ok_west = computePathPoint(
        src_longitude, src_latitude, dst_longitude, dst_latitude,
        kObstacleLineX - kCheckpointSpacingM, &cp_west_lon, &cp_west_lat);
    const bool ok_east = computePathPoint(
        src_longitude, src_latitude, dst_longitude, dst_latitude,
        kObstacleLineX + kCheckpointSpacingM, &cp_east_lon, &cp_east_lat);
    if (!ok_west || !ok_east) {
        return flyToAndHover(client, target_name, dst_longitude, dst_latitude,
                             altitude, final_yaw, arrival_tolerance);
    }

    // 机头对准穿越点（= 路径方向），前向相机正对障碍线。
    // 注意：方位角约定 0=正北、顺时针为正，与 kMatchYaw 同约定；实测不符可加偏移常量。
    const double guard_yaw = bearingDeg(src_longitude, src_latitude,
                                        dst_longitude, dst_latitude);

    const bool from_west = src_e < kObstacleLineX;
    if (from_west) {
        if (!flyToAndHover(client, "obstacle check west",
                           cp_west_lon, cp_west_lat, altitude, guard_yaw,
                           kCheckpointToleranceM)) return false;
        if (!guardCheckpoint(client)) return false;
        if (!flyToAndHover(client, "obstacle check east",
                           cp_east_lon, cp_east_lat, altitude, guard_yaw,
                           kCheckpointToleranceM)) return false;
        if (!guardCheckpoint(client)) return false;
    } else {
        if (!flyToAndHover(client, "obstacle check east",
