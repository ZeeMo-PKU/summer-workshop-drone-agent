...
constexpr double kTakeoffHeight = 1.57f;
...
// 拍照旋转角度
constexpr double kMatchYaw = -90.0f;
...
// 物理答题区 2和3的中点：(x=0, z=-2.71 m)，用于观察投球。
...
const std::string kRecognizeAnswerLayoutPrompt =
    "Identify the physical answer zones in the image. \
    Output two letters (A or B or C) in order from left to right, \
    No explanation.";
...
// Parse the model output into a target physical answer zone.
// Returns the first standalone A/B/C letter; falls back to A.
PhysicalAnswerZone pickZoneFromAnswer(const std::string& text, const std::string& answer) {
    ...
    if (text[0] == answer_letter) return PhysicalAnswerZone::Zone2;
    if (text[1] == answer_letter) return PhysicalAnswerZone::Zone3;
...
bool executeAnswerAndReturn(iking::drone::Client& client,
    ...
    const std::string recognized_order = recognize::recognizeImage(
        photo_zone_path, buildRecognizePrompt(kRecognizeAnswerLayoutPrompt), 30000);
    if (!recognized_order.empty()) {
        print("[round] recognized zone order from left to right: " + recognized_order);
    ...
    while (g_events.pop(event)) {
        ...
        if (event.message == "MATCH_FINISHED" ||
            ...
            const std::string recognized = recognize::recognizeImage(
                photo_path, buildRecognizePrompt("该无人机画面展示的是哪种应用场景？ B.森林火情巡查 C.水上活动巡查 A.电力设施巡检"), 30000);
            if (!recognized.empty()) {
