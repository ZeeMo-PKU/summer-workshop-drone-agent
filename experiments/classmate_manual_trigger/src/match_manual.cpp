 ...
 *
 * 默认 --dry-run：只打印裁判消息。
 * --execute：MATCH_STARTED 时闭合夹手、起飞到 1.57 m、飞往 B 触发区；
 *            SCENE_TRIGGER_SUCCEEDED 时在题目区拍照，再飞往答题区观察、投球并返回；
 *            MATCH_FINISHED / SAFETY_LINE_VIOLATION 时返航。
 * SIGUSR1 可在电脑端手动注入 MATCH_STARTED；SIGUSR2 注入 NEXT_ROUND_STARTED。
 */
...
constexpr char kCaptureDirectory[] =
    "/opt/iking/match_agent_manual/captures";
...
std::atomic<bool> g_controller_conflict{false};
volatile std::sig_atomic_t g_manual_first_requested = 0;
volatile std::sig_atomic_t g_manual_next_requested = 0;
...
void onSignal(int signal_number) {
    if (signal_number == SIGUSR1) {
        g_manual_first_requested = 1;
    } else if (signal_number == SIGUSR2) {
        g_manual_next_requested = 1;
    } else {
        g_stop.store(true);
    }
}
...
void drainManualRequests() {
    if (g_manual_first_requested != 0) {
        g_manual_first_requested = 0;
        enqueueManualEvent("MATCH_STARTED");
    }
    if (g_manual_next_requested != 0) {
        g_manual_next_requested = 0;
        enqueueManualEvent("NEXT_ROUND_STARTED");
    }
}

bool findOtherController(std::string& executable) {
    static const std::unordered_set<std::string> kOtherControllers = {
    ...
    while (!error && iterator != end) {
        ...
        if (pid == std::to_string(static_cast<long long>(getpid()))) {
            continue;
        }

        std::error_code link_error;
        const auto target =
            std::filesystem::read_symlink(entry.path() / "exe", link_error);
        if (link_error) continue;
        const std::string path = target.string();
        if (kOtherControllers.count(path) != 0) {
            executable = path;
            return true;
        }
    }
...
void eventLoop(iking::drone::Client& client, bool execute) {
    ...
    while (g_events.pop(event)) {
        ...
        if (event.message == "MATCH_FINISHED" ||
            ...
            if (!match_started || match_finished || answer_flow_started) {
            ...
            if (!match_started || match_finished || answer_flow_started) {
                print("[round] NEXT_ROUND_STARTED ignored in current state");
                continue;
            }
...
void requestReturnOnExit(iking::drone::Client& client, bool execute) {
    if (!execute) return;
    if (g_controller_conflict.load()) {
        print("[shutdown] another controller owns the aircraft; return command suppressed");
        return;
    }
...
void printUsage(const char* program) {
    ...
    std::cout
        << "  manual first round    send SIGUSR1 to this process\n"
        << "  manual next round     send SIGUSR2 to this process\n";
}
...
int main(int argc, char** argv) {
    ...
    config.client_id = "summer_workshop_match_agent_manual";
    ...
    if (execute && (!scene_camera_ready || !answer_camera_ready)) {
        print("[startup] execute refused: Front and PodVisibleLight must both be available");
        if (scene_camera_ready) {
    ...
    print("[startup] SDK connected; waiting for referee or manual round trigger");

    std::thread worker(eventLoop, std::ref(client), execute);
    auto last_controller_check = std::chrono::steady_clock::time_point{};
    while (!g_stop.load()) {
        drainManualRequests();
        const auto now = std::chrono::steady_clock::now();
        if (last_controller_check.time_since_epoch().count() == 0 ||
            now - last_controller_check >= 1s) {
            last_controller_check = now;
            std::string other_controller;
            if (findOtherController(other_controller)) {
                print("[safety] another controller appeared: " + other_controller);
                g_controller_conflict.store(true);
                g_stop.store(true);
            }
        }
        std::this_thread::sleep_for(200ms);
