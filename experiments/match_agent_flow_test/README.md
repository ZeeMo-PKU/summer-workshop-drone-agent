...
便携场地模式在首次 `MATCH_STARTED` 的地面预检中捕获启动经纬度、相对高度和机头航向：机头方向定义为场地 `+X`，场地 `+Z` 指向左侧。所有比赛点继续使用原场地米制坐标，但在运行时换算为当前场地经纬度。硬安全包络为启动点水平半径 `20 m`、相对高度 `-0.10 m` 至 `20 m`；任务巡航高度仍为 `7 m`。目标发送前和导航过程中都会复查包络，越界立即中止当前任务并返航。续轮起飞前还要求飞机回到已捕获启动点 `1 m` 以内，防止落地后误换锚点。
