...
- 投放后先以相对起飞点 0.3 米任务高度回到启动点，再使用 `returnToAnyPosition` 定点返航降落，避免默认返航高度造成额外爬升；比赛结束发生在 `TAKEOFF` 时会等待进入可返航模式。
- 便携场地以首次比赛开始时的实际经纬度和机头方向建立本地坐标系，硬安全包络为启动点水平半径 20 米、相对高度最高 20 米；触发区、答题观察点和投球区统一使用相对起飞点 0.3 米高度。
...
该实验版尚未合并到 `src/match.cpp`，也尚未完成新的 0.3 米真实飞行闭环验收。
